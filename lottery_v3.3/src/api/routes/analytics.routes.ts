...
// All analytics routes require authentication and view permissions
...
router.get('/games',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const summaryResult = await db.query(summaryQuery, [fromDate, toDate]);
...
router.get('/revenue',
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const metricsResult = await db.query(metricsQuery, [fromDate, toDate, currency]);
...
router.get('/players/:userId',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const winStatsResult = await db.query(
      `SELECT 
        COUNT(*) as total_wins,
...
router.get('/numbers',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const luckyResult = await db.query(
      `SELECT 
        selected_number,
        COUNT(*) as times_selected,
        COUNT(CASE WHEN is_winner THEN 1 END) as times_won,
