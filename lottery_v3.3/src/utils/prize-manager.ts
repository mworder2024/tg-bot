...
class PrizeManager {
  ...
  /**
   * Generate a random prize amount between 20,000 and 100,000 using VRF
   */
  generatePrize(gameId: string): { amount: number; vrfProof: string } {
    const vrfResult = VRF.generateRandomNumber(20000, 100000, `prize_${gameId}_${Date.now()}`);
    return {
  ...
  getUserWinnings(): UserWinnings[] {
    try {
      ...
      for (const winner of winners) {
        ...
        if (existing) {
          ...
          if (new Date(winner.timestamp) > existing.lastWin) {
            ...
            existing.username = winner.username; // Update to latest username
          }
        } else {
      ...
      return Array.from(userMap.values()).sort((a, b) => b.totalWinnings - a.totalWinnings);
    } catch (error) {
