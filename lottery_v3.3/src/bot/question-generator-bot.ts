  ...
  private setupHealthMonitoring(): void {
    ...
    setInterval(async () => {
      try {
        ...
        if (services.generator.status === 'unhealthy') {
          logger.error('Generator service is unhealthy:', services.generator);
        }

        if (services.integration.status === 'unhealthy') {
          logger.error('Integration service is unhealthy:', services.integration);
        }

        // Log healthy status periodically
        if (criticalAlerts.length === 0 && 
            services.generator.status === 'healthy' && 
            services.integration.status === 'healthy') {
