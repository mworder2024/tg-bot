...
import { ErrorMonitoringService } from '../src/services/error-monitoring.service.js';
...
describe('ErrorHandler', () => {
  ...
  describe('Error Classification', () => {
    ...
    it('should classify high severity errors correctly', () => {
      const paymentError = new PaymentError('Payment processing failed');
    ...
    it('should fail after max retry attempts', async () => {
      ...
      await expect(
        errorHandler.retryWithBackoff(
          ...
          { maxAttempts: 2, baseDelay: 10, maxDelay: 100, factor: 2 }
        )
...
describe('TelegramErrorHandler', () => {
  ...
  describe('Bot Error Handling', () => {
    it('should handle bot errors and send user-friendly messages', async () => {
      ...
      await telegramHandler.handleBotError(error, mockCtx);
    ...
    it('should provide appropriate error messages for different error types', async () => {
      ...
      await telegramHandler.handleBotError(paymentError, mockCtx);
      ...
      await telegramHandler.handleBotError(networkError, mockCtx);
      expect(mockCtx.reply).toHaveBeenLastCalledWith(
...
describe('OfflineErrorHandler', () => {
  ...
  describe('Error Queuing', () => {
    ...
    it('should provide queue status information', () => {
      ...
      expect(status.capabilities).toHaveProperty('hasLocalStorage');
      expect(status.capabilities).toHaveProperty('hasIndexedDB');
    });
  });
});
...
describe('ErrorMonitoringService', () => {
  ...
  describe('Error Capture', () => {
    ...
    it('should track metrics correctly', async () => {
      ...
      const error2 = new BlockchainError('Blockchain error');
      const context = { operation: 'test' };
  ...
  describe('Alert Configuration', () => {
    it('should add and remove alert configurations', () => {
      ...
      expect(() => monitoringService.removeAlertConfig('slack' as const)).not.toThrow();
    });
  });
});
...
describe('Integration Tests', () => {
  describe('Error Flow', () => {
    it('should handle complete error flow from occurrence to resolution', async () => {
      ...
      const error = new PaymentError('Integration test payment failure');
      const context = {
