import { TokenService, TokenReward } from './token.service.js';
import { RewardDistributionService } from './reward-distribution.service.js';
import { quizService, QuizSession, UserQuizStats } from '../quiz.service.js';
...
export class QuizTokenIntegrationService {
  ...
  private calculateAccuracy(session: QuizSession): number {
    ...
    let correct = 0;
    for (const [questionId, answer] of session.answers.entries()) {
  ...
  private calculateTimeBonus(session: QuizSession): number {
    ...
    const targetTime = session.questions.length * 60; // 1 minute per question
