...
export class PaymentService {
  ...
  async createPaymentRequest(
    ...
    try {
      ...
      // Update status to awaiting payment FIRST (critical fix for race condition)
      ...
      // Start monitoring for payment AFTER status update
  ...
  private async startPaymentMonitoring(paymentId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.logInfo('Payment monitoring started', logContext);
    } catch (error) {
      this.logger.logError(logContext, error as Error, {
  ...
  private async updatePaymentStatus(
    ...
    // Use Redis lock to prevent concurrent payment status updates (race condition fix)
    const lockKey = `${this.CACHE_PREFIX}lock:${paymentId}`;
    ...
    if (!lockAcquired) {
      throw new Error(`Payment ${paymentId} is being processed by another operation`);
    }

    try {
      // Perform the status update within the lock
      await this.db.query(
      ...
      // Log payment status update (simplified for compatibility)
      console.log(`Payment ${paymentId} status updated to ${status}:`, reason || 'No reason provided');
