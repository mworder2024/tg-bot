...
export class GameService {
  ...
  async endGame(gameId: string): Promise<string[]> {
    ...
    try {
      ...
      const vrfResult = VRF.generate(`${gameId}-${Date.now()}`);
  ...
  async addUserToGame(gameId: string, userId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.logGameEvent(logContext, {
        ...
        gameId,
        playerId: userId,
        metadata: { playerCount: game.currentPlayers }
      });
    } catch (error) {
      this.logger.logError(logContext, error as Error, {
  ...
  /**
   * Create VRF-based secure random number generator
   */
  private createSeededRandom(seed: string): () => number {
    let counter = 0;
    
    return () => {
      const vrf = VRF.generate(`${seed}_${counter++}`);
      // Convert hex value to a number between 0 and 1
      const hexValue = vrf.value.substring(0, 8);
