...
export class EnhancedGameService {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info('Enhanced game service initialized');
  }
  ...
  async createGame(
    ...
    try {
      ...
      if (options.type === 'paid' && options.entryFee) {
        ...
        const [pda] = (solanaIntegration as any).client.pdaHelper.getGamePDA(gameId);
        gamePDA = pda.toBase58();
        
        const [escrow] = (solanaIntegration as any).client.pdaHelper.getEscrowPDA(gameId);
  ...
  private async processEliminationRound(gameId: string): Promise<void> {
    ...
    try {
      ...
      // Generate VRF
      const vrfResult = await generateVRF(`${gameId}-round-${round}`);
  ...
  private async completeGame(gameId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.logGameEvent(logContext, {
        ...
        gameId,
        metadata: { winners: winnerTelegramIds }
      });
    } catch (error) {
      this.logger.logError(logContext, error as Error, {
