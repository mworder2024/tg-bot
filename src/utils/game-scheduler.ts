...
export class GameScheduler {
  ...
  /**
   * Check for scheduled games that can be activated when no active game is running
   */
  checkAndActivateScheduledGames(getCurrentGameFn: (chatId: string) => any): void {
    const now = Date.now();

    for (const schedule of this.schedules.values()) {
      if (!schedule.enabled) continue;

      // Check if there's currently no active game in this chat
      const currentGame = getCurrentGameFn(schedule.chatId);
      const hasActiveGame = currentGame && 
        (currentGame.state === 'WAITING' || 
         currentGame.state === 'NUMBER_SELECTION' || 
         currentGame.state === 'DRAWING');

      // If no active game and we have a scheduled game ready, activate it
      if (!hasActiveGame && schedule.nextRun.getTime() <= now + (30 * 60 * 1000)) { // 30 minutes ahead
        logger.info(`No active game found for chat ${schedule.chatId}, activating next scheduled game (${Math.ceil((schedule.nextRun.getTime() - now) / 60000)} minutes early)`);
        
        // Update the schedule to run now
        schedule.nextRun = new Date(now);
        this.runScheduledGame(schedule);
      }
    }
  }
