...
import { logger, StructuredLogger } from '../utils/structured-logger';
...
export class QuizGameService {
  ...
  private async endVoting(gameId: string): Promise<void> {
    ...
    try {
      ...
      if (tiedCategories.length > 1) {
        ...
        const vrfResult = VRF.generateRandomNumber(0, tiedCategories.length - 1, `${gameId}-category-tie`);
        const randomIndex = vrfResult.number;
      ...
      this.logger.logGameEvent(logContext, {
        event: 'voting_ended',
        gameId,
        metadata: { selectedCategory, voteResults }
      });
  ...
  private async nextQuestion(gameId: string): Promise<void> {
    ...
    try {
      ...
      const vrfResult = VRF.generateRandomNumber(0, questions.length - 1, `${gameId}-question-${game.currentRound}`);
      const questionIndex = vrfResult.number;
  ...
  private async startBonusRound(gameId: string, winnerId: string): Promise<void> {
    ...
    try {
      ...
      const vrfResult = VRF.generateRandomNumber(1, 100000, `${gameId}-bonus-reward`);
      const mworReward = vrfResult.number;
      ...
      for (let i = 0; i < 3; i++) {
        const vrfForQuestion = VRF.generateRandomNumber(0, allHardQuestions.length - 1, `${gameId}-bonus-q${i}`);
        const questionIndex = vrfForQuestion.number;
        bonusQuestions.push({
