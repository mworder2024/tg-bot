...
export class QuizGameManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info('Quiz game manager initialized', { 
  ...
  async createGame(
    ...
    try {
      ...
      this.logger.info('Quiz game created and managed', { 
        gameId: game.id, chatId, minPlayers, maxPlayers 
      });
  ...
  async cancelGame(gameId: string, reason: string = 'Cancelled by admin'): Promise<void> {
    try {
      ...
      this.logger.info('Quiz game cancelled', { 
        gameId, reason 
      });
    } catch (error) {
      this.logger.logError(this.logger.createContext(), error as Error, {
  ...
  private cleanupGame(gameId: string): void {
    ...
    this.logger.info('Quiz game cleaned up', { 
      gameId 
    });
  }
    ...
    try {
      ...
      if (gamesToCleanup.length > 0) {
        this.logger.info('Periodic cleanup completed', { 
          cleanedGames: gamesToCleanup.length 
        });
      }
    } catch (error) {
      this.logger.logError(this.logger.createContext(), error as Error, {
  ...
  shutdown(): void {
    ...
    this.logger.info('Quiz game manager shutdown completed');
  }
}