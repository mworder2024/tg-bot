...
interface WalletCommandContext extends Context {
  ...
  session?: { state?: string; [key: string]: any };
}
  ...
  bot.on('text', async (ctx, next) => {
    ...
    try {
      ...
      await ctx.reply(message, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '✅ Check Payment', callback_data: `check_verify:${verification.token}` }],
            [{ text: '❌ Cancel', callback_data: 'cancel_verification' }]
          ]
        }
      });
      ...
      await ctx.editMessageText(
        ...
        `_You can verify it again later if needed._`,
        {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                { text: '✅ Yes, Remove', callback_data: `confirm_remove:${walletAddress}` },
                { text: '❌ Cancel', callback_data: 'cancel_remove_wallet' }
              ]
            ]
          }
        }
