...
// import { PublicKey } from '@solana/web3.js'; // Unused import
...
export class PaidGameCommand {
    ...
    try {
      ...
      await ctx.reply(
        ...
        { 
          ...
          // disable_web_page_preview: true // Not supported in this context
        }
      );
    } catch (error) {
  ...
  async handleJoinPaidGame(ctx: Context, gameId: string) {
    try {
      ...
      const isInGame = game && (game as any).players?.includes(userId) || false;
      if (isInGame) {
   ...
   */
  private async getUserWallet(_userId: string): Promise<string | null> {
