...
export class LotteryProgramClient {
  ...
  async requestOraoVrf(
    ...
    const builder = this.program.methods
      ['requestOraoVrf' as any](gameId, round)
      .accounts({
        player: playerWallet,
        gameState: gamePDA,
        networkState,
  ...
  async fulfillOraoVrf(
    ...
    return await this.program.methods
      ['fulfillOraoVrf' as any](gameId, round)
      .accounts({
        authority: this.config.wallet.publicKey,
        gameState: gamePDA,
        randomness: randomnessAccount,
  ...
  async getVrfCostEstimate(expectedRounds: number): Promise<{
    ...
    const result = await this.oraoVrf.estimateGameVrfCost(expectedRounds);
    return {
      perRoundCost: result.perRequestSOL,
      totalCost: result.totalCostSOL,
      perRoundCostSOL: result.perRequestSOL,
      totalCostSOL: result.totalCostSOL
    };
  }
